...
for i in range(num_steps - 1):
    ...
    # 四步计算
    k1 = f(current_state, i*dt) * dt
    k2 = f(current_state + 0.5*k1, i*dt + 0.5*dt) * dt
    k3 = f(current_state + 0.5*k2, i*dt + 0.5*dt) * dt
    k4 = f(current_state + k3, i*dt + dt) * dt
    
    # 加权平均更新状态
    next_state = current_state + (k1 + 2*k2 + 2*k3 + k4) / 6.0
    
    # 存储结果
    x[i+1], y[i+1], vx[i+1], vy[i+1] = next_state
...
plt.tight_layout()
plt.savefig('RK_sun-earth.png', dpi=300)
...
plt.legend()
plt.show()
ani.save("2RK_anime.gif", fps=20, writer="imagemagick")